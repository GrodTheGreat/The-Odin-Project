def find_language_information(languages, language_name)
  ...
  languages.each do |key, value|
    return value if key == language_name
  end
end
...
def delete_information_about_language(languages, language_name, info_key)
  ...
  languages.each { |key, value| value.delete(info_key) if key == language_name }
end
...
def find_beautiful_languages(languages)
  ...
  languages.each_with_object({}) do |(key, value), result|
    result[key] = value if value[:is_beautiful?]
  end
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  #                 is_beautiful?: false }
  # }
  facts = languages.dig(language_name.to_sym, :facts)
  facts[fact_index] if facts
end
